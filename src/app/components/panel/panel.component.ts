 ...
 *
 * Content model:
 *   - `title` (required) and `subtitle` (optional) inputs render the header.
 *     They're inputs rather than projection slots so the panel owns the markup
 *     and can style it with its own encapsulated classes — emulated view
 *     encapsulation can't reach projected content (it keeps the consumer's
 *     scoping), which is why `subtitle` is not an `<ng-content>` slot.
 *   - default `<ng-content>`: panel body content.
 */
@Component({
  selector: 'app-panel',
  standalone: true,
